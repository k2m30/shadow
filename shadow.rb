...
svg.paths.each { |path| splitted_paths << path.split(size) }

save('splitted.svg', splitted_paths)

shadow_paths = []

d = properties['d']
w = find_width(splitted_paths)
h = properties['h']

p w
splitted_paths.each(&:organize!)
...
save_scad('shadow.scad', shadow_paths.map { |p| p.directions.map(&:finish) }.flatten)